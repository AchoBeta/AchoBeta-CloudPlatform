...
func createContainer(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[cmd] run container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_CREATE_FAIL)
		return
	} else if code == 2 {
		tlog.Errorf("[cmd] set container ssh pwd error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
	} else if code == 3 {
		tlog.Errorf("[db] insert container error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
	}
}
...
func getContainer(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[db] find container error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
	} else if code == 3 {
		tlog.Errorf("decode container error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
	}
}
...
func getContainers(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[db] find containers error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
	} else if code == 2 {
		tlog.Errorf("decode container error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
...
func removeContainer(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[cmd] delete container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_REMOVE_FAIL)
	} else if code == 2 {
		tlog.Errorf("[db] delete container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_REMOVE_FAIL)
	} else if code == 3 {
		tlog.Errorf("[db] update user containers error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
	}
}
...
func startContainer(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[cmd] start container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_START_FAIL)
	} else if code == 2 {
		tlog.Errorf("[db] start container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_START_FAIL)
	}
}
...
func stopContainer(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[cmd] stop container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_STOP_FAIL)
	} else if code == 2 {
		tlog.Errorf("[db] stop container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_STOP_FAIL)
	}
}
...
func restartContainer(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[cmd] restart container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_RESTART_FAIL)
	} else if code == 2 {
		tlog.Errorf("[db] restart container error ! msg: %s\n", err.Error())
		r.Error(handle.CONTAINER_RESTART_FAIL)
	}
}
...
func makeImage(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[cmd] make image from container error ! msg: %s\n", err.Error())
		r.Error(handle.IMAGE_CREATE_FAIL)
	} else if code == 2 {
		tlog.Errorf("[db] make image from container error ! msg: %s\n", err.Error())
		r.Error(handle.IMAGE_CREATE_FAIL)
	}
}
...
func getContainerLog(ctx context.Context, c *app.RequestContext) {
	...
	if code == 0 {
		...
		tlog.Errorf("[cmd] get container log error ! msg: %s\n", err.Error())
		r.Error(handle.INTERNAL_ERROR)
	}
}
